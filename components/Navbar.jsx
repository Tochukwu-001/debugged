...
const Navbar = () => {
  ...
  return (
    <nav className='flex items-center justify-between shadow-md px-5 py-3'>
      ...
      >
        <Avatar alt={session?.user?.name.slice(0, 1).toUpperCase()} src={session?.user?.image}/>
      </button>
      <Menu
        id="basic-menu"
        anchorEl={anchorEl}
      ...
      >
        ...
        <MenuItem onClick={handleClose}>
         <button onClick={() => signOut()} className='flex items-center gap-1 text-red-600 '>
          <BiSolidLogOutCircle />
          <span>logout</span>
      ...
      <Link href={"/auth/signin"} className='lg:flex hidden items-center gap-1 bg-blue-600 text-white px-6 
      py-2 rounded-full'>
      ...
      <div className={`bg-white h-full w-full lg:hidden absolute top-0 left-0 p-5 ${showNav ? "block" : "hidden"}`}>
        ...
        {
          !session &&  <Link href={"/auth/signin"} className='flex items-center justify-center gap-1 bg-blue-600 text-white px-6 py-2 rounded-full'>
          <FiUser className='text-xl' />
          <p>My Account</p>
        </Link>
        }
